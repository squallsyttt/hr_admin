...
use app\common\model\CompanyType;
use fast\Random;
...
class Ajax extends Api
{
    ...
    public function company_type()
    {
        ...
        $types = CompanyType::where('parent_id', 0)->with('children')->select();
        ...
        foreach ($types as $type) {
            ...
            $children = [];
            foreach ($type->children as $value) {
                $children[] =  [
                    'value' => $value->id,
                    'text' => $value->name,
                ];
     ...
     * nutui组件地区所需格式
     */
    public function nutuiArea()
    {
        $provinces = Area::where('pid', 0)->with('allChildren')->select();
        ...
        foreach ($provinces as $province) {
            ...
            foreach ($province->all_children as $city) {
                $cityItem = [
                    'id' => $city->id,
                    'value' => $city->name,
                    'text' => $city->name,
                ...
                foreach ($city->children as $district) {
                    ...
                    $cityItem['children'][] = $districtItem;
                }

                $item['children'][] = $cityItem;
            }
    ...
    {
        $params = $this->request->get();
        $map = config('site.' . ($params['mapType'] ?? ''));
        if (empty($map)) {
            $this->error('获取的字典不存在!!!!');
        }
        $this->success('请求成功', $map);
    }
}
